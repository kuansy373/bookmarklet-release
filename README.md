...
- コードが長く、モバイルではURL欄が途切れてしまうため、jsDelivr（CDN）での読み込みになっています。タグでバージョン管理しているため、最新リリースにするには使用者が能動的にタグを最新のものに書き換える必要があります。※タグと実際のコードの結びつきを正確に確認したい場合は、Releasesから該当タグが指しているコミットハッシュを参照ください。
