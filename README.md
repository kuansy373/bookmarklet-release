...
<pre><code>javascript:(function(){var s=document.createElement('script');s.src='https://cdn.jsdelivr.net/gh/kuansy373/novel-viewer-bookmarklet@v1.4.0/vertical-text-size-color.js';document.body.appendChild(s);})();
</code></pre>
<br>
最初実行したときは、このような感じです。
<br><br>
...
### 注意点
- コードが長く、モバイルではURL欄が途切れてしまうため、jsDelivr（CDN）での読み込みになっています。タグでバージョン管理しているため、最新リリースにするには使用者が能動的にタグを最新のものに書き換える必要があります。<br>※タグと実際のコードの結びつきを正確に確認したい場合は、「Releases」から該当タグが指しているコミットハッシュを参照ください。<br><br>
- ここに載せているjavascript:は常に最新のタグにしています。<br><br>
- v1.3.1を含むそれ以前のバージョンは、ブックマークレットを実行した際に新しいタブで開くのではなくもとのページを作り変えるため、広告が読み込まれる前に実行してしまうと宛先不明のメッセージがコンソールに大量に溜まることがあります。webページが広告まで読み込んでからブックマークレットを実行してください。<br><br>
- このリポジトリの名前は最初は「bookmarklet-release」でしたが、「novel-viewer-bookmarklet」に変更しました。（2025.12）<br><br>
